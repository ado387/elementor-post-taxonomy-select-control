...
class Custom_Post_Select extends \Elementor\Base_Data_Control {

	public function get_type()
	{
		return 'custom-post-select';
	}
	...
	public function content_template()
	{
		...
		<div class="elementor-control-field">
			...
			<div class="elementor-control-input-wrapper elementor-control-unit-5">
				...
				<select id="<?php echo $control_uid; ?>" class="post-select elementor-select2" type="select2" data-args="{{args}}">
					<# if ( data.controlValue ) { #>
					<option selected value="{{ data.controlValue.id }}">{{{ data.controlValue.name }}}</option>
					<# } #>
				</select>

				<input class="post-select-save-value" data-setting="{{data.name}}" type="hidden" />
			</div>
		</div>
